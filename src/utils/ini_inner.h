...
int ini_parse_stream(
        ini_reader reader, void *stream, ini_handler handler, void *user);
...
public:
    ...
    std::string
    Get(std::string section, std::string name, std::string default_value) const;
    ...
    // sensitive).
    bool
    GetBoolean(std::string section, std::string name, bool default_value) const;
...
inline long INIReader::GetInteger(
        std::string section, std::string name, long default_value) const {
...
inline double INIReader::GetReal(
        std::string section, std::string name, double default_value) const {
    std::string valstr = Get(section, name, "");
    const char *value = valstr.c_str();
    char *end;
...
inline float INIReader::GetFloat(
        std::string section, std::string name, float default_value) const {
    std::string valstr = Get(section, name, "");
    const char *value = valstr.c_str();
    char *end;
...
inline bool INIReader::GetBoolean(
        std::string section, std::string name, bool default_value) const {
    std::string valstr = Get(section, name, "");
    ...
    if (valstr == "true" || valstr == "yes" || valstr == "on" || valstr == "1")
        return true;
            ...
            valstr == "false" || valstr == "no" || valstr == "off"
            || valstr == "0")
        return false;
    else
...
inline bool INIReader::HasSection(const std::string &section) const {
    const std::string key = MakeKey(section, "");
    std::map<std::string, std::string>::const_iterator pos =
            _values.lower_bound(key);
...
inline bool
INIReader::HasValue(const std::string &section, const std::string &name) const {
    std::string key = MakeKey(section, name);
